...
public class KServices {
    ...
    public static int register(KService<?> kService, Class<?> clazz) {
        ...
        if (serviceId > 0) {
            throw new InitException("register duplicate" + serviceId);

        }
        serviceId = SERVICE_ID_GENERATOR.incrementAndGet();
        if (serviceId >= MAX_SERVICE_NUMS) {
            throw new InitException("exceed max limit " + MAX_SERVICE_NUMS);
        }
        ...
        try {
            ...
            throw new InitException("init SimpleService error", e);
        }
        return serviceId;
    }
