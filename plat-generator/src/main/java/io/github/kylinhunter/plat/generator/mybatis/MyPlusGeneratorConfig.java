...
public class MyPlusGeneratorConfig {
  ...
  public void init() {
    ...
    Map<String, String> mapperClasses =
        this.module.getTables().stream()
  ...
  public DataSourceConfig initDataSourceConfig() {
    ...
    dataSourceConfig.setUrl(
        "jdbc:mysql://localhost:3306/kp?serverTimezone=Asia/Shanghai&useUnicode=true"
  ...
  private static StrategyConfig initStrategyConfig(Module module) {
    ...
    TableFill tableFillCreate =
        new TableFill("sys_created_time", FieldFill.INSERT); // 创建时自动填充策略user数据库表
