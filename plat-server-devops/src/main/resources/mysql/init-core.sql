...
CREATE TABLE IF NOT EXISTS `kplat_tenant_user` (
  ...
  `sys_delete_flag` tinyint(1) NOT NULL DEFAULT 0 COMMENT '0 未删除 1删除',
  `sys_op_lock` int NULL COMMENT '乐观锁',
  `tenantId` varchar(64) NOT NULL DEFAULT '' COMMENT 'tenantId',
  `userId` varchar(64) NOT NULL DEFAULT '' COMMENT 'userId',
  `status` tinyint(4) NOT NULL DEFAULT 0 COMMENT '状态，预留',
  `description` varchar(255) NOT NULL DEFAULT '' COMMENT '描述',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_tenant_user` (`tenantId`,`userId`) COMMENT '唯一的code',
  KEY `idx_userId` (`userId`) USING BTREE COMMENT 'userId'
);
